...
func TestSsh_NewSSHClient_Good(t *core.T) {
	client := requireSSHClient(t, NewSSHClient(SSHConfig{Host: "web1", Port: 2200, User: "deploy"}))
...
func TestSsh_NewSSHClient_Bad(t *core.T) {
	client := requireSSHClient(t, NewSSHClient(SSHConfig{Host: "web1"}))
	core.AssertEqual(t, 22, client.port)
	core.AssertEqual(t, "root", client.user)
}

func TestSsh_NewSSHClient_Ugly(t *core.T) {
	client := requireSSHClient(t, NewSSHClient(SSHConfig{Host: "", Timeout: time.Nanosecond}))
...
func TestSsh_SSHClient_Connect_Bad(t *core.T) {
	...
	core.AssertContains(t, resultErrorMessage(result), "no authentication method")
}
...
func TestSsh_SSHClient_Run_Good(t *core.T) {
	...
	output := commandRunValue(result)
	core.AssertTrue(t, result.OK)
	core.AssertEqual(t, "web1\n", output.Stdout)
	core.AssertEmpty(t, output.Stderr)
...
func TestSsh_SSHClient_Run_Ugly(t *core.T) {
	...
	output := commandRunValue(result)
	core.AssertTrue(t, result.OK)
	core.AssertEmpty(t, output.Stdout)
	core.AssertEmpty(t, output.Stderr)
	core.AssertEqual(t, 0, output.ExitCode)
}
...
func TestSsh_SSHClient_RunScript_Good(t *core.T) {
	...
	result := client.RunScript(context.Background(), "echo script")
	output := commandRunValue(result)
	core.AssertTrue(t, result.OK)
...
func TestSsh_SSHClient_Upload_Ugly(t *core.T) {
	client := connectedSSHClient(t, map[string]string{"mkdir -p": ""})
	result := client.Upload(context.Background(), newReader(""), "/tmp/empty.txt", 0)
	core.AssertTrue(t, result.OK)
	core.AssertNotNil(t, client.client)
}
...
func TestSsh_SSHClient_FileExists_Bad(t *core.T) {
	...
	result := client.FileExists(context.Background(), "/tmp/missing.txt")
	core.AssertTrue(t, result.OK)
...
func TestSsh_SSHClient_Stat_Bad(t *core.T) {
	...
	result := client.Stat(context.Background(), "/tmp/missing.txt")
	core.AssertTrue(t, result.OK)
	info := result.Value.(map[string]any)
